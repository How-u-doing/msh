...
# check.pl
#    This program runs tests on msh and analyzes the results for
#    errors.
...
open(FOO, "msh.cc") || die "Did you delete msh.cc?";
$lines = 0;
...
# Conditionals
    ...
    [ 'Test COND4',
      '&& respects success status',
      'grep -cv NotThere ../msh.cc && echo Wanted',
      "$lines Wanted" ],

    [ 'Test COND5',
      '&& respects failure status',
      'grep -c NotThere ../msh.cc && echo Unwanted',
      '0' ],
...
# Pipelines
    ...
    [ 'Test PIPE12',
      ...
      "../msh -q cmd%%.sh; ps t $TTY | grep -m1 \"slee*p\" | wc -l",
      'Hello Bye 1',
      CMD_INIT => 'echo "echo Hello; sleep 2 & echo Bye" > cmd%%.sh'],
    ...
    [ 'Test PIPE14',
      ...
      'Hello 1',
      CMD_INIT => 'echo "echo Hello; sleep 0.4" > cmd%%.sh',
      CMD_CLEANUP => "echo \"sleep 0.2 ; ps t $TTY | grep -cm1 \\\"slee*p\\\"\" > cmd%%b.sh; ../msh -q cmd%%b.sh",
...
# Redirection
    ...
    [ 'Test REDIR7',
      'redirection and pipeline',
      'tr hb HB < f%%.txt | sort | ../msh -q cmd%%.sh',
...
# check for a ton of existing msh processes
$me = `id -un`;
chomp $me;
open(SH61, "ps uxww | grep '^$me.*msh' | grep -v grep |");
$nmsh = 0;
$nmsh += 1 while (defined($_ = <SH61>));
close SH61;
if ($nmsh > 5) {
    print STDERR "\n";
    print STDERR "${Red}**** Looks like $nmsh ./msh processes are already running.\n";
    print STDERR "**** Do you want all those processes?\n";
    print STDERR "**** Run 'killall -9 msh' to kill them.${Off}\n\n";
}
    ...
    eval {
        do {
            ...
            if (defined($size_limit) && $size_limit_file && @$size_limit_file) {
                my($len) = 0;
                $out = run_msh_pipe($out, fileno(OR), $size_limit);
...
sub run (@) {
    ...
    my($info) = run_msh($cmd, "stdin" => $stdin, "stdout" => $outfile, "time_limit" => $time_limit, "size_limit" => 1000, "dir" => "out", "nokill" => 1, "delay" => 0.05, "int_delay" => $opts{CMD_INT_DELAY});

    if ($opts{CMD_CLEANUP}) {
        if ($opts{CMD_CAREFUL_CLEANUP}) {
            my($infox) = run_msh("{ " . $opts{CMD_CLEANUP} . "; } >>$outfile 2>&1", "time_limit" => $time_limit, "dir" => "out", "stdin" => "/dev/stdin", "stdout" => "/dev/stdout");
